import os
import sqlite3
from flask import Flask, render_template, redirect, flash, Response, request,url_for,g
...
class RegistrationForm(FlaskForm):
    ...
    aadhar = StringField('Aadhar No', validators=[DataRequired()])
...
def download_history():
    ...
    cursor.execute('SELECT user.user_id, User.username, User.mobile,User.aadhar,User.vehicleNumber,booking_details.slot_no, booking_details.place, booking_details.from_datetime,booking_details.to_datetime FROM User, booking_details WHERE  booking_details.user_id = User.user_id;')
    history_download = cursor.fetchall()
    print("history_download :",history_download)
    close_db(conn)
    ...
    if history_download:
        ...
        for row in history_download:
            csv_writer.writerow(row)
        csv_data = output.getvalue()
        output.close()

        # Return CSV data as a response to trigger download
        return Response(
            csv_data,
            mimetype="text/csv",
            headers={"Content-disposition":
                            "attachment; filename=users_history.csv"}
        )
    else:
        flash('Unable to Generate User History!!!', 'success')
        return redirect('/history')


    flash('Downloaded History successfully', 'success')
    return redirect('/history')
        ...
        if user_id:
            ...
            if form.validate_on_submit():
                ...
                if(balance<10):
                    ...
                    return redirect('/dashboard')
                try:
                    cursor.execute('INSERT INTO booking_details (user_id, username, place, slot_no, from_datetime, to_datetime,entry_datetime,exit_datetime) VALUES (?, ?, ?, ?, ?, ?,?,?)',
                                   (user_id, username, place, slot_no, from_datetime, to_datetime,0,0))
                    conn.commit()
                    ##update balance
    ...
    try:
        ...
        if len(bookings) > 0:
            ...
            try:
                conn = get_db()
                cursor = conn.cursor()
                cursor.execute('SELECT entry_datetime,exit_datetime FROM booking_details WHERE user_id = ?', (action,))
                ...
                if(entry_time==0):
                    current_time = datetime.utcnow()
                # Count the number of booked slots for the current time
                    cursor.execute('UPDATE booking_details SET entry_datetime = ? WHERE user_id = ?', ((current_time.strftime('%Y-%m-%d %H:%M:%S'), action)))
                    print("entry time updated")
                elif(exit_time==0):
                    current_time = datetime.utcnow()
                # Count the number of booked slots for the current time
                    cursor.execute('UPDATE booking_details SET exit_datetime = ? WHERE user_id = ?', ((current_time.strftime('%Y-%m-%d %H:%M:%S'), action)))
                    print("exit time updated")


                # cursor.execute('UPDATE booking_details SET entry_datetime = ? WHERE user_id = ?', (int(datetime("now")), action))
                # cursor.execute('INSERT booking_details SET entry_datetime = CURRENT_TIMESTAMP WHERE user_id = ?', (action,))
